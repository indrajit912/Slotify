...
from app.utils.token import generate_api_token, generate_admin_verification_code
...
@admin_bp.route('/generate-avc', methods=['GET', 'POST'])
@admin_required
def generate_avc_page():
    """
    Allow admins to generate an Admin Verification Code (AVC)
    for a specific user email. The AVC securely encodes both
    the admin's and user's email addresses.
    """
    if request.method == 'POST':
        user_email = request.form.get('user_email', '').strip().lower()

        if not user_email:
            flash("Please enter a valid user email address.", "danger")
            return redirect(url_for('admin.generate_avc_page'))

        # Optional: check that the user does not already exist
        existing_user = User.query.filter_by(email=user_email).first()
        if existing_user and existing_user.email_verified:
            flash("This user email is already verified. No code needed.", "warning")
            return redirect(url_for('admin.generate_avc_page'))

        # ✅ Generate the encoded AVC
        try:
            avc_code = generate_admin_verification_code(
                admin_email=current_user.email,
                user_email=user_email
            )

            logger.info(f"AVC generated by {current_user.email} for {user_email}")
            flash("Admin Verification Code generated successfully!", "success")

            return render_template(
                "generate_avc_page.html",
                avc_code=avc_code,
                user_email=user_email,
                admin_email=current_user.email
            )
            ...
            logger.exception(f"Error generating AVC: {e}")
            flash("Failed to generate Admin Verification Code. Please try again.", "danger")
            return redirect(url_for('admin.generate_avc_page'))

    # For GET request: show the form
    return render_template("generate_avc_page.html")
